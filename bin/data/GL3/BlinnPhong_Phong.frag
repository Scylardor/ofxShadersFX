...
vec4 directional_light(in int lightIndex, in vec3 normal) {
    ...
    lightDir = normalize(eyeSpaceVertexPos.xyz - cameraSpaceLightPos);
    /* The ambient term will always be present */
    ambient = material.ambient * lights.light[lightIndex].ambient/* + materialEmission */;
    outputColor += ambient;
...
vec4 calc_lighting_color(in vec3 normal) {
  ...
  for (int i = 0; i < lightsNumber; i++) {
    ...
    if (lights.light[i].position.w == 0.0) {
      // if (lights.light[i].position.xyz == cameraSpaceLightPos) {
      // 	//	lightingColor = vec4(1.0);
      // } else {
      //lightingColor = vec4(1.0, 0., 0., 1.);
	//      }
